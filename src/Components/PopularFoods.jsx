    ...
    <div className="container mx-auto mt-20">
      ...
      <Fade cascade damping={0.1}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-3">
          {foods.map((food) => (
            <Link key={food._id} to={`/foods-purchase/${food._id}`}>
              <div
                className="bg-white hover:scale-105 duration-300 hover:border-blue-400 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-md overflow-hidden"
              >
                <div className="p-4">
                  {/* Buyer Details */}
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-600 dark:text-gray-300">
                    <p>
                      Ordered:{" "}
                      <span className="font-medium">{food.orderedCount}</span>{" "}
                      times
                    </p>
                    <p>
                      Available:{" "}
      ...
      <div className="flex items-center justify-center my-3">
        <Link to={`/all-foods`}>
          <button className="btn bg-violet-600 text-white px-10">
            See More
          </button>
        </Link>
      </div>
