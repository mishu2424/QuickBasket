      ...
      >
        <span
          className={`absolute top-20 right-2 px-3 py-1 text-xs ${
            ...
            food_category === "Vegetables" && "text-emerald-800 bg-emerald-100"
          } ${
        ...
        <div className="px-4 py-3 mt-5 grow">
          <div className="flex items-center justify-between">
            ...
            {location.pathname!=='/' && food?.orderedCount && (
              <span className="absolute top-0 left-23 text-[10px] font-semibold bg-sky-300 text-white p-2 rounded-full">Ordered Count:{food.orderedCount || 0}</span>
            )}
            {/* {location.pathname!=='/' && !food?.orderedCount && (
              <span className="absolute top-0 left-27 text-[10px] font-semibold bg-sky-300 text-white p-2 rounded-full">Ordered Count: 0</span>
            )} */}
          </div>
